-- 20260718000000_courier_features.sql

-- Add advanced tracking fields to shipments table
ALTER TABLE public.shipments
ADD COLUMN IF NOT EXISTS assigned_courier_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS is_hazmat BOOLEAN NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS volumetric_weight NUMERIC(10,2);

-- Recreate the create_shipment_with_payment RPC to accept the new fields
DROP FUNCTION IF EXISTS public.create_shipment_with_payment(
  UUID, TEXT, JSONB, JSONB, JSONB, NUMERIC, BOOLEAN, BOOLEAN, TEXT, DATE, NUMERIC
);
DROP FUNCTION IF EXISTS public.create_shipment_with_payment(
  p_user_id UUID,
  p_service TEXT,
  ...
  WHILE NOT v_inserted AND v_retry_count < v_max_retries LOOP
    ...
    BEGIN
      INSERT INTO public.payment_transactions (
        shipment_id, method, amount, currency, reference, status
      ) VALUES (
        v_shipment_id, 'pending', p_total_amount, 'USD', v_pay_ref, 'pending'
