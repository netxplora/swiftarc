...
function drawFooter(doc: jsPDF, pageW: number, pageH: number, includeLegal = true) {
  ...
  doc.text(
    "100 Logistics Way, Suite 400 | San Francisco, CA 94107, USA | +1 (800) 555-0199 | swiftarc.app",
    40,
    pageH - 45,
  );
  ...
  if (includeLegal) {
    ...
    doc.text(
      "This document is electronically generated and is valid without a physical signature. " +
        "Subject to SwiftArc Carrier Agreement available at swiftarc.app/legal.",
      40,
      pageH - 25,
    );
  }
...
function generateBusinessDocument(opts: BusinessDocumentOptions) {
  ...
  if (opts.sender) {
    ...
    doc
      .setTextColor(BRAND_NAVY)
      .setFontSize(11)
      .setFont("helvetica", "bold")
      .text("Sender:", 50, contentY + 20);
  ...
  if (opts.recipient) {
    ...
    doc
      ...
      .setFontSize(11)
      .setFont("helvetica", "bold")
      .text("Recipient:", 310, contentY + 20);
    doc.setFontSize(10).setFont("helvetica", "normal").setTextColor(TEXT_MUTED);
    doc.text(
      [
        opts.recipient.contact,
        opts.recipient.line1,
        [opts.recipient.city, opts.recipient.zip, opts.recipient.country]
          .filter(Boolean)
          .join(", "),
  ...
  if (opts.requiresSignature) {
    contentY += 40;
    doc
      .setDrawColor(BRAND_NAVY)
      .setLineWidth(0.5)
      .line(40, contentY + 40, 250, contentY + 40);
...
export function generateShippingLabel(shipment: any) {
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    [dest.line1 || "", `${dest.city}, ${dest.postal_code || ""}`, dest.country_code || ""].filter(
      Boolean,
    ),
    5,
    66,
...
export function generateCommercialInvoice(inv: InvoiceInput) {
  ...
  doc
    ...
    .setFontSize(11)
    .setFont("helvetica", "bold")
    .text("Shipped to (Receiver):", 310, 130);
  doc.setFontSize(10).setFont("helvetica", "normal").setTextColor(TEXT_MUTED);
  doc.text(
    [
  ...
  const desc = inv.customsInfo?.description || `Logistics - ${inv.service}`;
  doc.text(desc, 50, rY);

  if (inv.customsInfo?.hs_code) {
    doc.setFont("helvetica", "normal").setFontSize(8).setTextColor(TEXT_MUTED);
    doc.text(`HS Code: ${inv.customsInfo.hs_code}`, 50, rY + 12);
  }
  ...
  doc
    .setTextColor(BRAND_NAVY)
    .setFont("helvetica", "bold")
    .text(`$${lineTotal.toFixed(2)}`, 500, rY);
...
export function generateCustomsDeclaration(c: CustomsInput) {
  ...
  doc.text(
    `Tracking: ${c.trackingNumber} | Date: ${format(new Date(), "MMM dd, yyyy")}`,
    pw - 40,
    90,
    { align: "right" },
  );
  ...
  doc.text(
    [c.origin.contact, c.origin.line1, `${c.origin.city}, ${c.origin.zip}`, c.origin.country],
    40,
    225,
    { lineHeightFactor: 1.5 },
  );
  ...
  const desc = c.customsInfo?.description || "Commercial Goods - General";
  doc.text(desc, 50, rY);

  if (c.customsInfo?.hs_code) {
    doc.setFont("helvetica", "normal").setFontSize(8).setTextColor(TEXT_MUTED);
    doc.text(`HS Code: ${c.customsInfo.hs_code}`, 50, rY + 12);
  }
  ...
  doc
    .setFont("helvetica", "bold")
    .setTextColor(BRAND_NAVY)
    .text("Total Gross Weight:", pw - 220, rY + 65);
...
export function generateShippingReceipt(shipment: any) {
  generateBusinessDocument({
    ...
    details: [
      ...
      {
        label: "Created At",
        value: format(new Date(shipment.created_at || Date.now()), "MMM dd, yyyy HH:mm"),
      },
    ],
...
export function generateProofOfDelivery(shipment: any, podData: any) {
  generateBusinessDocument({
    ...
    details: [
      ...
      {
        label: "Delivery Date",
        value: format(new Date(podData?.timestamp || Date.now()), "MMM dd, yyyy HH:mm"),
      },
...
export function generateBillingInvoice(inv: FinancialInvoiceInput) {
  ...
  doc.text(`Date Issued: ${format(new Date(inv.issueDate), "MMM dd, yyyy")}`, pw - 40, 92, {
    align: "right",
  });
  doc.text(`Due Date: ${format(new Date(inv.dueDate), "MMM dd, yyyy")}`, pw - 40, 106, {
    align: "right",
  });
  ...
  inv.lineItems.forEach((li) => {
    ...
    doc
      .setTextColor(BRAND_NAVY)
      .setFont("helvetica", "bold")
      .text(`${inv.currency === "USD" ? "$" : ""}${li.amount.toFixed(2)}`, 500, rY);
    rY += 30;
  });
  ...
  doc
    .setTextColor(BRAND_NAVY)
    .text(`${inv.currency === "USD" ? "$" : ""}${inv.subtotal.toFixed(2)}`, 500, tY + 25);

  doc.setTextColor(TEXT_MUTED).text(`Tax:`, 380, tY + 45);
  doc
    .setTextColor(BRAND_NAVY)
    .text(`${inv.currency === "USD" ? "$" : ""}${inv.tax.toFixed(2)}`, 500, tY + 45);
...
export function generateShippingManifest(
  ...
  doc
    ...
    .setFontSize(11)
    .setFont("helvetica", "bold")
    .text("Dispatch Information", 50, 130);
  doc.setFontSize(10).setFont("helvetica", "normal").setTextColor(TEXT_MUTED);
  ...
  shipments.forEach((s) => {
    ...
    const dest =
      typeof s.destination === "string" ? JSON.parse(s.destination) : s.destination || {};
