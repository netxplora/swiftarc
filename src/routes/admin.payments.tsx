    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Manage transactions, digital wallets, and payment gateways.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-border pb-4">
        ...
        <TabButton active={tab === "wallets"} onClick={() => setTab("wallets")} icon={Wallet}>
          Crypto Wallets
        </TabButton>
...
function TransactionsTab() {
  ...
  const verifyMut = useMutation({
    mutationFn: (args: { id: string; action: "verify" | "reject"; note?: string }) =>
      verifyMutFn({ data: args }),
    onSuccess: (res) => {
  ...
  const filtered = (q.data ?? []).filter(
    (t: any) =>
      ...
      t.shipments?.tracking_number.toLowerCase().includes(search.toLowerCase()),
  );
    ...
    <Card>
      <CardContent className="p-0">
        <div className="border-b border-border p-4">
          <Input
            placeholder="Search references or tracking IDs…"
            value={search}
      ...
      <CardContent className="p-0">
        ...
        {q.isLoading ? (
          ...
          <div className="divide-y divide-border">
              ...
              <div key={w.id} className="flex items-center justify-between p-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`h-2 w-2 rounded-full ${w.status === "active" ? "bg-success" : "bg-destructive"}`}
                    />
                  </div>
            ...
            {(q.data ?? []).length === 0 && (
              <div className="p-10 text-center text-muted-foreground">No wallets configured.</div>
            )}
          </div>
        )}
      </CardContent>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {wallet ? (
          ...
          <Button size="sm" className="bg-navy-deep text-cream hover:bg-navy">
      ...
      <DialogContent>
        ...
        <form onSubmit={submit} className="space-y-4 pt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Network (e.g. Ethereum)</Label>
              <Input required value={network} onChange={(e) => setNetwork(e.target.value)} />
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="e.g. Send via ERC-20 only"
            />
          </div>
          <div className="space-y-1.5">
            <Label>Status</Label>
      ...
      <CardContent className="p-0">
        <div className="border-b border-border p-4">
          ...
          <p className="text-sm text-muted-foreground">
            Enable or disable payment methods globally.
          </p>
        </div>
        {q.isLoading ? (
          <div className="p-10 text-center">
