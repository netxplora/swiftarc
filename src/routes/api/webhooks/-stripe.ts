...
// @ts-expect-error - Module might not be properly typed in this version
        ...
        if (reference) {
          ...
          if (txn.status !== "verified") {
            ...
            const { data: shipmentRel } = await supabaseAdmin
              .from("shipments")
              .select(
                "id, tracking_number, service, user_id, payment_transactions(amount, currency, reference)",
              )
              .eq("payment_transaction_id" as any, txn.id)
              .maybeSingle();
            ...
            if (shipmentRel) {
              ...
              if (profile?.email && process.env.RESEND_API_KEY) {
                ...
                const t = Array.isArray(shipmentRel.payment_transactions)
                  ? shipmentRel.payment_transactions[0]
                  : shipmentRel.payment_transactions;
